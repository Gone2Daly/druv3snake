...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = tuple((random.randrange(self.size_num-2)+1,random.randrange(self.size_num-2)+1))

            # Choose a random direction index
            start_direction_index = random.randrange(4)
    ...
    def init_food(self):
        ...
        self.current_available_food_positions = set(zip(*np.where(self.world == 0))).difference(set(self.snake.blocks))
        if not self.custom:
            # Choose a random position from available now
            chosen_position = tuple(random.sample(self.current_available_food_positions,1)[0])
        else:
        ...
        self.food_position = tuple(chosen_position)
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if tuple(new_snake_head+DIRECTIONS[self.snake.current_direction_index]) in self.snake.blocks:
                print("Meet tail")
                self.snake.alive = False
            ...
            if self.food_position in self.snake.blocks:
                # Remove old food
                print("Meet food")
                print(len(self.current_available_food_positions))
                self.world[self.food_position[0],self.food_position[1]] -= self.FOOD
                self.food_position = 0
                # Add tail again
                # Note: all Snake coordinates should be tuples(Y, X)
                new_head1 = tuple(self.snake.blocks[0] + DIRECTIONS[self.snake.current_direction_index])
                self.snake.blocks = [new_head1] + self.snake.blocks
